...
router.post("/register", async function (req, res) {
    ...
    try {
        ...
        const injectedHtml = Mustache.render(verifyEmailTemplate, {
            username,
            verify_link: process.env.RUN_IN_SSL_MODE === "true" ? `https://${process.env.HTTP_HOSTNAME}:${process.env.HTTPS_LISTEN_PORT}/api/verify/${vToken}` : `http://${process.env.HTTP_HOSTNAME}:${process.env.HTTP_LISTEN_PORT}/api/verify/${vToken}`
        })
        // send the email
        ...
        return res.status(200).json({
            Status: "OK",
            Messages: "Account has been created successfully!\nPlease check your email to confirm your account"
        })
    } catch (saveError) {
        ...
        if (saveError.toString().includes("Unique")) {
            return res.status(400).json({
                Status: "Client Error",
                Messages: "That username or email is already exist!,Please try different username or email."
            })
        }
        return res.status(500).json({
            Status: "Server Error!",
            Messages: "an server error has occurred!,Please try again later."
        })
    }
})
